...
SELECT job_id, SUM(salary) AS soma FROM hr.employees
GROUP BY job_id
HAVING job_id <> 'it_prog'
ORDER BY soma DESC;
...
SELECT department_id, AVG(salary) AS media, COUNT(*) AS numero_de_funcionarios
FROM hr.employees
...
SELECT last_name, DATE(hire_date) FROM hr.employees
